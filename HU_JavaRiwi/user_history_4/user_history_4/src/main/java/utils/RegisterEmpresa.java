...
public class RegisterEmpresa {
    ...
    public void eliminarEmpleado (int idEliminar, Map<String , Empleado> mapaglobal, List<Empleado> listaGlobal) {
        

        if (mapaglobal.containsKey(String.valueOf(idEliminar))) {
            System.out.println("Empleado con ID " + idEliminar + " no encontrado. No se puede eliminar.");
            return;
        }

        listaGlobal.removeIf(emp -> emp.getId() == idEliminar);

        for (Empresa e : listaEmpresas) {

            if (e.getEmpleados() != null) {
                e.getEmpleados().removeIf(emp -> emp.getId() == idEliminar);
            }
        }

        mapaglobal.remove(String.valueOf(idEliminar));

        System.out.println("¡Empleado con ID " + idEliminar + " eliminado correctamente de todas las fuentes!");
